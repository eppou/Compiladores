#include "graph.hpp"
#include <limits> // Incluir esta biblioteca para usar std::numeric_limits<int>::max()

int allocator::Graph::return_vertex_with_less_interferences() {
    int less_interferences = std::numeric_limits<int>::max();
    ...
    for (auto const& [key, vertex] : this->vertexes) {
        if (vertex->active && vertex->id >= this->available_registers) {
            ...
            for (auto const& [_, interference] : vertex->interferences) {
                if (interference->active) {
                    interferences++;
                }
            }

            if (interferences < less_interferences || (interferences == less_interferences && vertex->id < less_interferences_vertex->id)) {
